...
Sun1 = Body("Sun1", M_sun, Vector(-d/2, 0), Vector(0,  v_sun))
Sun2 = Body("Sun2", M_sun, Vector(d/2,  0), Vector(0, -v_sun))

#smaller planet orbiting both suns
r_planet = 2 * d
v_planet = math.sqrt(G * (2*M_sun) / r_planet)

Planet = Body("Planet", 5.972e27, Vector(r_planet, 0), Vector(0, v_planet))
...
animate_trajectories(positions, interval=1)
